...
function formatCompetitor(competitor, label) {
  return [
    ...
    assertString(
      pick(competitor, ["name", "username", "display_name", "displayName"]),
      `${label}.name`,
    ),
...
function renderBoard(game, game_mode) {
  if (game_mode !== "4winning") {
    return [];
  }
...
function calculateDurationInSeconds(createdAt, updatedAt) {
  return Math.max(
    0,
    Math.floor((new Date(updatedAt).getTime() - new Date(createdAt).getTime()) / 1000),
  );
}
